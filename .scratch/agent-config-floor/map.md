...
A spec, ready to hand to implementation, for a **floor-raising toolkit** that brings any repo in the org up to a minimum agent-config baseline — the documentation, instructions and skills Claude Code and GitHub Copilot actually need to work well — runnable self-serve or with platform-team assistance, never enforced.
...
5. **Effectiveness is judged, not measured.** The quality bar is whether the setup is genuinely good — complete, correct, in paths the agents read — assessed qualitatively, with an LLM as the judge. No telemetry, no instrumentation, no before/after study. **Token spend is the lowest priority** and is not a success criterion; it may be noted, never optimised for.
...
- **Telemetry and quantitative measurement** — per-repo token attribution, usage APIs, before/after studies, instrumentation of any kind. Ruled out: effectiveness is judged qualitatively instead. Closed [What telemetry can attribute token spend and quality to a repo](issues/02-what-telemetry-attributes-cost-and-quality-to-a-repo.md) as mis-scoped.
- **Optimising token consumption as a goal** — explicitly the lowest priority. Good setup may reduce spend; that is a side effect, not the target.
